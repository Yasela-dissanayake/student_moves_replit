...
export default function setupVoucherRoutes(app: express.Application, storage: IStorage, customAI: typeof CustomAIProvider) {
    ...
    try {
      ...
      for (const voucher of vouchers) {
        ...
        try {
          ...
          try {
            // Get company directly from database using pool
            ...
            console.error(`Error fetching company ${voucher.companyId}:`, companyErr);
          }
      ...
      const validVouchers = await Promise.all(
        vouchers
          ...
          .map(async (voucher) => {
            ...
            try {
              const companyQuery = await pool.query(
                'SELECT id, name, logo, business_type as "businessType", categories, allows_bookings as "allowsBookings" FROM voucher_companies WHERE id = $1',
                [voucher.companyId]
              );
              ...
              console.error(`Error fetching company for voucher ${voucher.id}:`, error);
            }
    ...
    try {
      ...
      const enrichedVouchers = await Promise.all(
        vouchers.map(async (voucher) => {
          ...
          try {
            const companyQuery = await pool.query(
              'SELECT id, name, logo FROM voucher_companies WHERE id = $1',
              [voucher.companyId]
            );
  ...
  router.get('/:id', async (req, res) => {
    try {
      ...
      // Get company details directly from database
      let company = null;
      try {
        const companyQuery = await pool.query(
          `SELECT id, name, logo, address, city, postcode, phone, email, website, 
          business_type as "businessType", categories, allows_bookings as "allowsBookings", 
          operating_hours as "operatingHours", average_rating as "averageRating"
          FROM voucher_companies WHERE id = $1`,
          [voucher.companyId]
        );
  ...
  router.post('/', checkAuth(), upload.array('images', 5), async (req, res) => {
    try {
      ...
      try {
        const companyQuery = await pool.query(
          `SELECT id, name, owner_id as "ownerId", verified, logo, business_type as "businessType"
           FROM voucher_companies WHERE id = $1`,
          [companyId]
        );
  ...
  router.patch('/:id', checkAuth(), upload.array('images', 5), async (req, res) => {
    try {
      ...
      try {
        const companyQuery = await pool.query(
          `SELECT id, name, owner_id as "ownerId", verified
           FROM voucher_companies WHERE id = $1`,
        ...
        console.error(`Error fetching company with ID ${existingVoucher.companyId}:`, error);
      }
        ...
        redemptions.map(async (redemption) => {
          ...
          if (voucher && voucher.companyId) {
            try {
              ...
              console.error(`Error fetching company with ID ${voucher.companyId}:`, error);
            }
          }
  ...
  router.post('/:id/book', checkAuth(), async (req, res) => {
    try {
      ...
      try {
        const companyQuery = await pool.query(
          `SELECT id, name, allows_bookings as "allowsBookings", booking_lead_hours as "bookingLeadHours", 
      ...
      const enrichedBookings = await Promise.all(
        bookings.map(async (booking) => {
          ...
          if (voucher && voucher.companyId) {
            try {
              const companyQuery = await pool.query(
                `SELECT id, name, logo, address, city, phone
                 FROM voucher_companies WHERE id = $1`,
                [voucher.companyId]
              );
      ...
      const enrichedSavedVouchers = await Promise.all(
        savedVouchers.map(async (saved) => {
          ...
          if (voucher && voucher.companyId) {
            try {
              const companyQuery = await pool.query(
                `SELECT id, name, logo, business_type as "businessType"
                 FROM voucher_companies WHERE id = $1`,
                [voucher.companyId]
              );
