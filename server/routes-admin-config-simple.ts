...
router.get('/', async (req, res) => {
  try {
    console.log("Simple admin config GET route accessed");
...
// PUT /api/admin-config-simple - Update admin configuration
router.put('/', async (req, res) => {
  try {
    console.log("Simple admin config PUT route accessed");
...
// POST /api/admin-config-simple/test-update - Test update functionality
router.post('/test-update', async (req, res) => {
  try {
    console.log("Test update route accessed");
    ...
    console.log("Test config data received for update");
    ...
    if (existingResult && existingResult.length > 0) {
      ...
      return res.status(200).json({
        success: true,
        data: updatedResult[0],
        message: "Configuration updated successfully via test endpoint"
      });
    } else {
    ...
    return res.status(500).json({
      success: false,
      message: 'Failed to test update admin configuration',
      error: error instanceof Error ? error.message : String(error)
    });
  }
});
