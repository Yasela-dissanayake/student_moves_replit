...
export async function analyzePropertyDocument(
  ...
  try {
    ...
    switch (analysisMode) {
      case 'lease-analysis':
        prompt = 'Analyze this lease agreement in detail. Identify key terms, responsibilities of landlord and tenant, rent amount, security deposit, lease duration, renewal options, maintenance responsibilities, and any unusual or potentially unfavorable clauses for the tenant. Format your analysis under clear headings.';
        break;
      case 'compliance-check':
        prompt = 'Evaluate if this document complies with standard UK rental regulations. Check for appropriate clauses regarding deposits, notice periods, landlord\'s obligations for repairs and tenant safety, prohibited fees, and right-to-rent verification. Note any potential non-compliance issues.';
        break;
      case 'summarization':
        prompt = 'Provide a concise summary of this document in approximately 250 words. Highlight the main purpose, key parties involved, important dates, financial obligations, primary responsibilities, and any notable special conditions.';
        break;
      case 'risk-assessment':
        prompt = 'Perform a risk assessment on this rental document. Identify potential risks for the tenant, including financial exposures, ambiguous clauses, unbalanced responsibilities, excessive penalties, or legal vulnerabilities. Rate each risk (Low/Medium/High) and provide mitigation advice.';
        break;
      case 'right-to-rent':
        prompt = 'Analyze this document for right-to-rent verification purposes. Identify the type of document, whether it appears to be a valid right-to-rent document under UK law, the validity period if applicable, and any limitations or conditions noted.';
        break;
      case 'guarantor':
      ...
      case 'contract-review':
        ...
        break;
      case 'custom':
        prompt = customPrompt || 'Analyze this document and provide a detailed assessment.';
        break;
      case 'general':
      default:
        prompt = 'Analyze this document comprehensively. Identify the document type, main purpose, key parties involved, important terms and conditions, financial obligations, dates, and any notable or unusual provisions. Organize your analysis under clear headings.';
    }
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a specialized real estate document analyzer that excels at extracting and analyzing information from property-related documents. You're helping students and landlords understand rental agreements, compliance issues, and other property documents."
        },
        {
          role: "user",
          content: [
...
export async function extractDocumentInfo(
  ...
  try {
    ...
    let prompt = '';
    let systemPrompt = 'You are a specialized document information extractor that can accurately pull structured data from various types of documents. Extract the information in a structured JSON format.';
    ...
    switch (documentType) {
      case 'lease':
        ...
        break;
      case 'utility_bill':
        prompt = 'Extract the following information from this utility bill as structured JSON data: provider name, account number, customer name, service address, billing period, amount due, due date, and payment options. Include a "services" array listing individual services and charges if available.';
        break;
      case 'passport':
        prompt = 'Extract the following information from this passport as structured JSON data: full name, nationality, passport number, date of birth, sex/gender, place of birth, issue date, expiry date, and issuing authority. Include a "documentAuthenticityIndicators" field with any visible security features.';
        break;
      case 'id':
        prompt = 'Extract the following information from this ID document as structured JSON data: document type, ID number, full name, date of birth, address (if present), issue date, expiry date, and issuing authority. Include a "documentAuthenticityIndicators" field with any visible security features.';
        break;
      case 'bank_statement':
        prompt = 'Extract the following information from this bank statement as structured JSON data: bank name, account holder name, account number (last 4 digits only for privacy), statement period, opening balance, closing balance, and total deposits and withdrawals. Include a "summary" field with any notable patterns.';
        break;
      ...
      case 'guarantor_agreement':
        ...
        break;
      default:
        prompt = 'Extract all relevant information from this document in a structured JSON format. Identify the document type first, then extract fields appropriate for that document type.';
    }
    ...
    throw new Error(`Failed to extract document information: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
`;
    ...
    throw new Error(`Failed to verify right to rent document: ${error instanceof Error ? error.message : String(error)}`);
  }
}