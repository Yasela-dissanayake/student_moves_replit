...
const TenantRiskAssessment: React.FC = () => {
  ...
  const parseAssessmentData = (assessment: TenantRiskAssessment): RiskAssessmentData => {
    try {
      return JSON.parse(assessment.assessmentData);
    } catch (e) {
      console.error('Error parsing assessment data:', e);
      return {
        overallRiskScore: parseInt(assessment.overallRiskScore) || 0,
  ...
  const handleTenantSelect = (value: string) => {
    ...
    // If on tenant history tab, we'll need the tenant assessments
    if (activeTab === "tenant") {
      queryClient.invalidateQueries({ queryKey: ['/api/tenant-risk/assessments', tenantId] });
    }
  };
        ...
        <CardContent>
          <Tabs value={activeTab} onValueChange={handleTabChange}>
            <TabsList className="mb-4">
              ...
              <TabsTrigger value="tenant" disabled={!selectedTenantId}>Tenant History</TabsTrigger>
            </TabsList>
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectGroup>
                        ...
                        {isLoadingTenants ? (
                          <SelectItem value="loading-placeholder" disabled>Loading tenants...</SelectItem>
              ...
              {selectedTenantId && (
                <div className="mt-4">
                  <Alert variant="default" className="border">
                    ...
                    <AlertTitle>Selected Tenant</AlertTitle>
                    <AlertDescription>
                      {tenants?.find((t: Tenant) => t.id === selectedTenantId)?.name || 'Unknown tenant'}
                    </AlertDescription>
                  </Alert>
                </div>
              )}
              ...
              {isLoadingRecent ? (
                ...
                <div className="space-y-4">
                  {recentAssessments.map((assessment: TenantRiskAssessment) => {
                    const tenantName = tenants?.find((t: Tenant) => t.id === assessment.tenantId)?.name || 'Unknown tenant';
                ...
                <div className="text-center py-8">
                  ...
                  <h3 className="text-lg font-medium">No recent assessments</h3>
            ...
            {/* Tenant History Tab - Always rendered but conditionally shows content */}
            <TabsContent value="tenant">
              {selectedTenantId ? (
                <>
                  <div className="mb-4">
                    <h3 className="text-lg font-semibold">
                      Assessment History for {tenants?.find((t: Tenant) => t.id === selectedTenantId)?.name || `Tenant #${selectedTenantId}`}
                    </h3>
                    <p className="text-sm text-muted-foreground">
                      View all risk assessments for this tenant
                    </p>
                  </div>
                  ...
                  {isLoadingTenantAssessments ? (
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-muted-foreground mt-2">
                        This tenant has no risk assessments yet.
                      </p>
                      <Button 
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground mt-2">
                    Please select a tenant first to view their risk assessment history.
                  </p>
                  <Button 
                    onClick={() => setActiveTab("new")} 
                    className="mt-4"
        ...
        <Dialog open={isVerifyDialogOpen} onOpenChange={setIsVerifyDialogOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Add verification notes and confirm the assessment.
              </DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter className="flex justify-between">
              ...
              >
                {verifyAssessmentMutation.isPending ? 'Verifying...' : 'Verify Assessment'}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
